...
export default function HotelsAdmin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editing ? `${API_BASE}/admin/hotels/${editing.id}` : `${API_BASE}/admin/hotels`;
      ...
      if (res.ok) {
        ...
        setShowForm(false);
        setEditing(null);
        setFormData({ name: '', location: '', rating: '', description: '', images: [] });
      }
    } catch (error) {
      console.error(error);
    }
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Are you sure?')) {
      try {
        await fetch(`${API_BASE}/admin/hotels/${id}`, {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` },
        });
        fetchHotels();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-green-50 to-emerald-100 p-8 flex items-center justify-center relative overflow-hidden">
      {/* Background decorative elements */}
      <div className="absolute inset-0 opacity-5">
        <div className="absolute top-20 left-20 w-72 h-72 bg-gradient-to-r from-green-400 to-emerald-500 rounded-full blur-3xl"></div>
      ...
      <div className="max-w-6xl mx-auto w-full relative z-10">
        <div className="flex justify-between items-center mb-12 bg-white/80 backdrop-blur-lg rounded-3xl p-8 shadow-2xl border border-white/20">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-4xl font-bold mt-12 bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">Manage Hotels</h1>
              <p className="text-gray-600 mt-2 text-lg">Create and manage hotel listings for your destinations</p>
            </div>
          </div>
          <button
        ...
        {showForm && (
          <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/30 p-8 mb-12 animate-in slide-in-from-top-5 duration-500">
            <div className="flex items-center mb-6">
              <div className="w-12 h-12 bg-gradient-to-r from-green-600 to-emerald-600 rounded-2xl flex items-center justify-center mr-4 shadow-lg">
                <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                </svg>
              </div>
              <h2 className="text-2xl font-bold text-gray-900">{editing ? 'Edit' : 'Add'} Hotel</h2>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full p-4 border-2 border-gray-200 rounded-2xl focus:border-green-600 focus:ring-4 focus:ring-green-600/10 transition-all duration-300 bg-white/50 backdrop-blur-sm"
                  />
                </div>
                <div>
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    placeholder="Describe this hotel"
                    value={formData.description}
                    onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                    className="w-full p-4 border-2 border-gray-200 rounded-2xl focus:border-green-600 focus:ring-4 focus:ring-green-600/10 transition-all duration-300 bg-white/50 backdrop-blur-sm resize-none"
                    rows={4}
                  />
                </div>
                <div className="md:col-span-2">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full p-4 border-2 border-dashed border-gray-300 rounded-2xl focus:border-green-600 focus:ring-4 focus:ring-green-600/10 transition-all duration-300 bg-white/50 backdrop-blur-sm file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-green-600 file:text-white hover:file:bg-green-700"
                    />
                  </div>
                </div>
              </div>
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/30 overflow-hidden">
          <table className="w-full">
            <thead className="bg-gradient-to-r from-green-600 to-emerald-600">
              <tr>
                <th className="px-8 py-5 text-left text-sm font-bold text-white uppercase tracking-wider">Hotel</th>
                <th className="px-8 py-5 text-left text-sm font-bold text-white uppercase tracking-wider">Location</th>
                <th className="px-8 py-5 text-left text-sm font-bold text-white uppercase tracking-wider">Rating</th>
                <th className="px-8 py-5 text-left text-sm font-bold text-white uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200/50">
              {hotels.map((hotel, index) => (
                <tr key={hotel.id} className="hover:bg-gradient-to-r hover:from-green-50 hover:to-emerald-50 transition-all duration-300 animate-in fade-in slide-in-from-bottom-2" style={{ animationDelay: `${index * 100}ms` }}>
                  <td className="px-8 py-6">
                    <div className="flex items-center">
                      ...
                      <div>
                        <div className="text-lg font-semibold text-gray-900">{hotel.name}</div>
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center">
                      ...
                      <span className="text-gray-900 font-medium">{hotel.rating}</span>
                    </div>
                  </td>
          ...
          {hotels.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-gray-500">Start by adding your first hotel</p>
            </div>
          )}
        </div>
      </div>
